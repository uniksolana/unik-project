...
### 6.2. Shared Transaction Notes (Database Obfuscation)

**Algorithm:** AES-256-GCM
**Key derived from:** The public blockchain transaction signature (SHA-256)

Notes that a payer attaches when sending money (e.g., *"March rent payment"*) need to be readable by both sender and recipient. Since true E2EE between wallets requires a complex key exchange, UNIK uses the transaction signature to obfuscate the note in the database:

**How does it work?**

1. The payer writes a note when sending funds.
2. The transaction is sent to Solana and generates a **public signature** (`signature`, e.g., `5UHk7...wQ3P`).
...
5. Within the UNIK UI, both sender and recipient use the known transaction signature to seamlessly decrypt the note locally.

**Privacy level:**
Shared notes are obfuscated in our database against casual observation. As server administrators, we only see ciphertext. However, because Solana is a public network, any advanced user who knows the exact public transaction signature could theoretically derive the key and read the note. Do not use this field for passwords or sensitive state secrets; use it for payment concepts.
...
4. **Automatic Residual Percentage:** The main wallet always receives the remaining percentage. If you assign 30% to a third party, your main wallet automatically drops to 70%. The system UI and Smart Contract both validate that the sum of external rules never exceeds 100% to prevent blockchain calculation errors.
...
- **ATA Activation:** If the recipient has never received the selected token (USDC/EURC), the system detects that their *Associated Token Account (ATA)* doesn't exist and offers the sender to create it in a prior step (minimal cost of ~0.002 SOL). This prevents failed transactions and improves the user experience, as the sender automatically covers the creation rent.
...
| ✅ **Idempotent ATA** | Associated token accounts are automatically created if they don't exist, ensuring seamless transactions without network errors. |
| 🔄 **Replay Protection** | Solana provides native consensus-level replay protection. Our `processed_signatures` table acts as application-level protection to prevent the UI/backend from fulfilling the same payment order twice. |
